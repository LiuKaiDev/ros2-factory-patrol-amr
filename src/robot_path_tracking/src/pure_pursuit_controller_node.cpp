...
class PurePursuitControllerNode final : public rclcpp::Node {
 public:
  PurePursuitControllerNode() : Node("pure_pursuit_controller_node") {
    ...
    csv_logger_ = std::make_unique<robot_path_tracking::TrackingCsvLogger>(
        enable_csv_logging, csv_output_path, "pure_pursuit", get_logger());
 ...
 private:
  void Tick() {
    ...
    const double metric_heading_error = robot_path_tracking::HeadingError(path_yaw, yaw);
    ...
    if (robot_path_tracking::IsFinalWaypointReached(
      ...
      PublishError(nearest_index, lateral_error, metric_heading_error, 0.0);
      WriteMetric(pose.position.x, pose.position.y, yaw, nearest_pose.position.x,
                  nearest_pose.position.y, path_yaw, cmd, lateral_error, metric_heading_error,
    ...
    const double target_heading_error =
        robot_path_tracking::NormalizeAngle(std::atan2(dy, dx) - yaw);
    const double curvature =
        2.0 * std::sin(target_heading_error) / std::max(lookahead_distance_, 0.05);
    ...
    PublishError(nearest_index, lateral_error, metric_heading_error, cmd.linear.x);
    WriteMetric(pose.position.x, pose.position.y, yaw, nearest_pose.position.x,
                nearest_pose.position.y, path_yaw, cmd, lateral_error, metric_heading_error,
  ...
  void WriteMetric(const double x, const double y, const double yaw, const double ref_x,
    ...
    sample.controller = "pure_pursuit";
